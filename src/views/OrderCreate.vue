<template>
  <div>
    <!-- Main Container -->
    <div class="order-create-container row">
      
      <!-- Left Container: Tabs and Items -->
      <div class="order-create-left-container col-8">
        <!-- Category Tabs -->
        ...
        <q-tab-panels v-model="tab" animated transition-prev="jump-up" transition-next="jump-down" class="order-create-tab-list">
          <q-tab-panel v-for="category in categories" :key="category.id" :name="category.name" class="q-pa-md">
            <div class="row q-pa-sm q-col-gutter-md order-create-item-card-container">
              <div v-for="item in category.items" :key="item.id" class="order-create-item-card q-pa-xs col-2">
                <q-btn push @click="openDialog(item)" class="order-create-list-buttons q-pa-none">
                  <q-card flat class="order-create-list-cards">
                    ...
                    <q-card-section class="order-create-item-image q-pa-none">
      ...
      <div class="order-create-right-container col-4 q-pl-md">
        <!-- Transaction List -->
        <div class="order-create-list-area">
          <div class="order-create-list-header q-py-xs">
            <p class="q-ma-none text-bold">
              Transaction <span class="text-primary text-h6">'{{ currentTransaction }}'</span> Items
            </p>
          </div>
          <div class="order-create-scrollable-list">
            <q-table
              ...
              hide-bottom
              dense
              class="order-create-scrollable-table"
            >
              <!-- Custom Body Cells -->
              <template v-slot:body-cell-name="props">
                <q-td :props="props" class="order-create-name-column">
                  <p class="q-ma-none text-left order-create-wrap-text">
                    {{ props.row.label }} 
              ...
              <template v-slot:body-cell-price="props">
                <q-td :props="props" class="order-create-price-column">
                  <p class="q-ma-none text-center order-create-wrap-text">
                    ${{ props.row.price }}
                  </p>
                </q-td>
              </template>
              <template v-slot:body-cell-quantity="props">
                <q-td :props="props" class="order-create-quantity-column">
                  <div class="row items-center justify-center">
                    <q-btn flat class="order-create-list-counter-buttons" icon="remove" @click="confirmDecrease(props.row)" />
                    <div class="order-create-list-quantity q-border rounded-borders q-mx-xs">
                      <p class="order-create-list-counter-input q-px-sm q-ma-none order-create-wrap-text">
                        {{ props.row.quantity }}
                      </p>
                    </div>
                    <q-btn flat class="order-create-list-counter-buttons" icon="add" @click="increaseItemQuantity(props.row)" />
                  </div>
                </q-td>
              </template>
              <template v-slot:body-cell-actions="props">
                <q-td :props="props" class="order-create-actions-column auto-width">
        ...
        <!-- Customer Actions -->
        <div class="order-create-customer-area q-pa-md">
        ...
        <!-- Action Buttons -->
        <div class="order-create-action-area q-pa-md">
          <div class="row q-col-gutter-sm">
            <div class="col-4">
              <q-btn label="RESET" push class="order-create-action-buttons bg-negative text-white full-width q-py-sm" @click="confirmCancel"></q-btn>
            </div>
            <div class="col-4">
              <q-btn label="SWITCH" push class="order-create-action-buttons bg-warning full-width q-py-sm" @click="switchTransaction"></q-btn>
            </div>
            <div class="col-4">
              <q-btn label="SUBMIT" push class="order-create-action-buttons bg-positive text-white full-width q-py-sm" @click="submitTransaction"></q-btn>
            </div>
          </div>
        </div>
      </div>
    ...
    <q-dialog v-model="dialog" backdrop-filter="brightness(50%)">
      <q-card class="order-create-dialog-cards">
        <q-bar>
          <q-space></q-space>
          <q-btn dense flat icon="close" v-close-popup>
            <q-tooltip class="bg-white text-primary">Close</q-tooltip> 
        ...
        <q-card-section class="column items-center q-gutter-y-sm">
          ...
          <div v-if="selectedItem?.laundry_price !== null" class="row items-center q-pa-sm">
            ...
            <q-btn push class="order-create-counter-buttons" icon="remove" @click="decreaseQuantity('Laundry')" />
            <q-input outlined v-model="quantities.Laundry" class="order-create-counter-input q-pa-none q-mx-md" />
            <q-btn push class="order-create-counter-buttons" icon="add" @click="increaseQuantity('Laundry')" />
          </div>
          ...
          <div v-if="selectedItem?.dryclean_price !== null" class="row items-center q-pa-sm">
            ...
            <q-btn push class="order-create-counter-buttons" icon="remove" @click="decreaseQuantity('DryClean')" />
            <q-input outlined v-model="quantities.DryClean" class="order-create-counter-input q-pa-none q-mx-md" />
            <q-btn push class="order-create-counter-buttons" icon="add" @click="increaseQuantity('DryClean')" />
          </div>
          ...
          <div v-if="selectedItem?.pressing_price !== null" class="row items-center q-pa-sm">
            ...
            <q-btn push class="order-create-counter-buttons" icon="remove" @click="decreaseQuantity('PressingOnly')" />
            <q-input outlined v-model="quantities.PressingOnly" class="order-create-counter-input q-pa-none q-mx-md" />
            <q-btn push class="order-create-counter-buttons" icon="add" @click="increaseQuantity('PressingOnly')" />
          </div>
          ...
          <div v-if="selectedItem?.others_price !== null" class="row items-center q-pa-sm">
            ...
            <q-btn push class="order-create-counter-buttons" icon="remove" @click="decreaseQuantity('Others')" />
            <q-input outlined v-model="quantities.Others" class="order-create-counter-input q-pa-none q-mx-md" />
            <q-btn push class="order-create-counter-buttons" icon="add" @click="increaseQuantity('Others')" />
          </div>
        </q-card-section>
        <q-card-actions>
          <q-btn class="full-width order-create-dialog-buttons" label="Add Item" color="primary" @click="addToTransaction" /> 
        </q-card-actions>
      </q-card>
    </q-dialog>
    ...
    <q-dialog v-model="showCustomerInfoDialog" backdrop-filter="brightness(50%)">
      <q-card class="order-create-dialog-customer">
        <q-bar>
          <q-space></q-space>
          <q-btn dense flat icon="close" v-close-popup>
            <q-tooltip class="bg-white text-primary">Close</q-tooltip> 
        ...
        <q-card-actions>
          <q-btn class="full-width order-create-dialog-buttons" label="Add Details" color="primary" v-close-popup /> 
        </q-card-actions>
      </q-card>
    </q-dialog>
    ...
    <q-dialog v-model="showInstructionsDialog" backdrop-filter="brightness(50%)">
      <q-card class="order-create-dialog-instructions">
        <q-bar>
          <q-space></q-space>
          <q-btn dense flat icon="close" v-close-popup>
            <q-tooltip class="bg-white text-primary">Close</q-tooltip> 
        ...
        <q-card-section class="column items-center q-gutter-y-md">
          ...
          <q-btn class="order-create-dialog-buttons" label="Add Instruction" color="secondary" @click="addInstruction" /> 
        </q-card-section>
        <q-separator />
        <q-card-section>
          <div>
        ...
        <q-card-actions>
          <q-btn class="full-width order-create-dialog-buttons" label="Submit Instructions" color="primary" v-close-popup /> 
        </q-card-actions>
      </q-card>
    </q-dialog>
    ...
    <q-dialog v-model="showUploadImageDialog">
      <q-card>
        ...
        <q-card-section>
          <div class="order-create-upload-image flex items-end q-col-gutter-sm">
            <div class="col order-create-auto-width">
              <q-file
                filled
                bottom-slots
...
import {
  ...
  getMaxOrderId,
  uploadPhoto,
} from "@/../supabase/api/createOrder.js";
...
const submitTransaction = async () => {
  try {
    const currentImage = currentTransaction.value === "A" ? cinfo_imageA.value : cinfo_imageB.value;
    if (!currentImage) {
      throw new Error("Upload Customer Information");
    }
    ...
    const currentCustomerData = currentTransaction.value === "A" ? customerA.value : customerB.value;
    const contactPerson = currentCustomerData.name;
    const contactPersonNo = currentCustomerData.contactNo;
    const deliveryAddress = currentCustomerData.address;
    const remarks = currentCustomerData.remarks;

    const dateTime = new Date().toISOString();
    const readyBy = new Date(readyByDate.value).toISOString(); // Use selected date
    const status = "Pending";
    const order = await insertOrder(orderNo, dateTime, readyBy, status, photoUrl);
